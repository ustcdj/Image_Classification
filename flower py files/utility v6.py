...
def save_checkpoints(model, arch, lr, epochs, input_size, hidden_units, class_to_idx, checkpoint_path):
    ...
    print('Checkpoint saved in ', checkpoint_path + 'checkpoint.pth')
...
# Labeling
def labeling(flowers_names):
    with open(flowers_names, 'r') as f:
...
def predict(image_path, model, topk, device):
    ...
    with torch.no_grad():
        images = torch.from_numpy(image_path)
        ...
        top_probs, indices = torch.topk(ps, topk)
        top_probs = [float(prob) for prob in top_probs[0]]
        inv_map = {v: k for k, v in model.class_to_idx.items()}
        top_classes = [inv_map[int(index)] for index in indices[0]]
...
parser.add_argument('-p', action='store',
                    dest='path', help='path of directory', required=True)
parser.add_argument('--save_dir', action='store',
                    dest='cp_path', default='checkpoints/', help='checkpoint path')
parser.add_argument('--gpu', action='store_true',
                    default=False, dest='switch', help='Set a switch to use GPU')
parser.add_argument('--arch', action='store',
                    dest='arch', default='densenet121', choices={"densenet121", "vgg16"}, help='select one model of the network')
parser.add_argument('--learning_rate', action='store',
                    nargs='?', default=0.002, type=float, dest='learning_rate', help='enter learning rate')
parser.add_argument('--epochs', action='store',
                    dest='epochs', default=1, type=int, help='enter (int) number of epochs to train')
parser.add_argument('--hidden_units', action='store',
                    nargs=1, default=500, dest='hidden_units', type=int, help='enter hidden units of the network')
